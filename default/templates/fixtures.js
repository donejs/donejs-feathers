...
fixture("OPTIONS <%= feathersUrl %>/auth/token", function(request, response, headers, ajaxSettings){
...
fixture("POST <%= feathersUrl %>/auth/token", function(request, response, headers, ajaxSettings){
  return {
    token: '<%= jwtToken %>',
    data: {
      <%= idProp %>: 1,
...
fixture("OPTIONS <%= feathersUrl %>/auth/local", function(request, response, headers, ajaxSettings){
  return response(200, "", `HTTP/1.1 204 No Content
    Access-Control-Allow-Origin: *
    Access-Control-Allow-Methods: GET,HEAD,PUT,PATCH,POST,DELETE
    Access-Control-Allow-Headers: content-type
...
fixture("POST <%= feathersUrl %>/auth/local", function(request, response, headers, ajaxSettings){
  return {
    token: '<%= jwtToken %>',
    data: {
      <%= idProp %>: 1,
